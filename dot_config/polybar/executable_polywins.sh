...
active_text_color=$ACTIVE_FG
active_bg=$ACTIVE_BG
active_underline=$UNDERLINE
#"#5294e2"

inactive_text_color="#d3dae3"
inactive_bg=
inactive_underline=
...
#		done
	WNAME=$(i3-msg -t get_workspaces | jq --arg MONITOR $MONITOR -r '.[] | select(.visible==true) | select(.output==$MONITOR) | .name')
	wmctrl -d | grep "$WNAME" | cut -f1 -d" "
}
...
generate_window_list() {
	...
	wins=$(wmctrl -l | cut -c 13 | grep -c $active_workspace)
	if [ $wins -gt 0 ] 
	then 
	...
	while read -r wid ws cname_cls host title; do
		...
		if [ "${#w_name}" -gt "$char_limit" ]; then
			w_name="$(echo "$w_name" | cut -c1-$((char_limit-1)))…"
	                ...
	                pad=$((($char_limit - ${#w_name}) / 2))
			w_name=$(printf "%-${pad}s%s%-${pad}s" "" "$w_name" "")
		fi
